...
name      =  "Keyes Thomas"  -- replace with your name
...
ins k d (Leaf key ddata)
  | key < k = Branch Empty (Leaf k d) key ddata
  | key > k = Branch (Leaf k d) Empty key ddata
  | key == k = Leaf k d

ins k d (Branch l r key ddata)
  | key < k = Branch l (ins k d r) key ddata
  | key > k = Branch (ins k d l) r key ddata
